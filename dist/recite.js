...
var qPointer = 0, isReciting = false;
...
function start_recite() {
    ...
    if ($("#recite").hasClass("none-display") || (!isReciting))
        show("#recite");
    else
        Err.error_display("背诵已开始！", 4000, "❕");
    if (!isReciting)
        next_question();
}
function submit_question() {
    ...
    isReciting = false;
    show("#result");
    var ans = $("#rc-ans").attr("disabled", "disabled").val(), questionNow = questions[qPointer], corrAns = questionNow.get_corrAns(), simiRelaxed = simi_quick(ans, corrAns), simiStrict = simi_levenshtein(ans, corrAns), simiTtl = (prStrict * simiStrict + (10 - prStrict) * simiRelaxed) / 10, recScore = ((simiTtl * 100 >= exSimi) ?
        ((simiTtl * 100 - exSimi) / (100 - exSimi)) : ((exSimi <= 50) ? ((simiTtl * 100 - exSimi) / exSimi) : ((simiTtl * 100 <= 2 * exSimi - 100) ? 0 : ((simiTtl * 100 - exSimi) / (100 - exSimi))))) * 10, simiColor = (simiTtl * 100 >= 99) ? "#008800" : ((simiTtl * 100 >= exSimi) ? "#888800" : "#ff0000");
    $("#rs-cor").text(corrAns);
    $("#rs-ans").text(ans);
