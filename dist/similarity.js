...
function simi_quick(a, b) {
    var arra = charcode_arr(a).sort((a, b) => a - b), arrb = charcode_arr(b).sort((a, b) => a - b), la = arra.length, lb = arrb.length, lmax = Math.max(arra.length, arrb.length), lcommon = 0, ia = 0, ib = 0;
    while (true) {
        if (ia >= la || ib >= lb)
            break;
        if (arra[ia] > arrb[ib])
